...
function setDeviceStatusTheme(status) {
    const device = $("handheldDevice");
    if (!device) return;
    device.classList.remove("status-ready", "status-normal", "status-suspicious", "status-high-risk");
    const key = status === "High-Risk" ? "status-high-risk" : status === "Suspicious" ? "status-suspicious" : status === "Normal" ? "status-normal" : "status-ready";
    device.classList.add(key);
}
...
function updateDashboard(event) {
    ...
    setDeviceStatusTheme(event.status);
    updateDeviceClock();
    $("deviceStatus").textContent = event.status.toUpperCase();
    $("deviceScore").textContent = `${score.toFixed(1)}%`;
    $("deviceTemp").textContent = `${event.temperature}°C`;
    $("deviceHumidity").textContent = `${event.humidity}%`;
    $("deviceGas").textContent = event.gas_level;
    $("deviceSignal").textContent = event.signal_level;
    $("deviceGps").textContent = Number.isFinite(Number(event.latitude)) ? "LOCKED" : "SEARCH";
    $("deviceTarget").textContent = event.target === "Narcotics + Explosives" ? "NARC + EXP" : event.target.toUpperCase();
...
async function clearLog() {
    ...
    `;
    setDeviceStatusTheme("READY");
    updateDeviceClock();
    $("deviceStatus").textContent = "READY";
    $("deviceScore").textContent = "--";
    $("deviceTemp").textContent = "--";
    $("deviceHumidity").textContent = "--";
    $("deviceGas").textContent = "--";
    $("deviceSignal").textContent = "--";
    $("deviceGps").textContent = "LOCKED";
    $("deviceTarget").textContent = "NARC + EXP";
